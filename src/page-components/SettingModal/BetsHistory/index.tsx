...
import { ArrowIcon, BetWinIcon, BetLoseIcon } from '@/page-components/Icon';
...
const BetsHistory = ({ isMobile, caAddress, setWinRatio }: BetsHistoryProps) => {
  ...
  const { data, loading, refetch } = useCaHolderBingoInfoQuery({
    variables: {
      dto: { skipCount: 0, maxResultCount: 10, caAddresses: [caAddress] },
    },
    onCompleted: (data) => {
      setPage(page + 1);
      setDataLength(dataLength + data.caHolderBingoInfo.data.length);
      setBetList(groupBetsByDate(data.caHolderBingoInfo.data, betList));
      setWinRatio(`${data.caHolderBingoInfo.statics[0].totalWins}/${data.caHolderBingoInfo.statics[0].totalPlays}`);
    },
  });
  const totalRecordCount = data?.caHolderBingoInfo?.totalRecordCount || 0;
